...
from src.utils import weights_init_xavier


class ALAD(BaseModel):
    def __init__(self, in_features: int, out_features: int, latent_dim: int, device):
        super(ALAD, self).__init__()
        self.device = device
        self.latent_dim = latent_dim
        self.in_features = in_features
        self.out_features = out_features
        self.D_xz = DiscriminatorXZ(in_features, 128, latent_dim, negative_slope=0.2, p=0.5)
        self.D_xx = DiscriminatorXX(in_features, 128, negative_slope=0.2, p=0.2)
        self.D_zz = DiscriminatorZZ(latent_dim, latent_dim, negative_slope=0.2, p=0.2)
        self.G = Generator(latent_dim, in_features, negative_slope=1e-4)
        self.E = Encoder(in_features, latent_dim)
